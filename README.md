...
- `SCD41` 和 `SPS30` 现在共用一条 I2C 总线
- 默认接线明显更简单
- 不再需要 `GPIO15 / GPIO16 / GPIO17 / GPIO18` 这组 PMS7003 控制/UART 引脚
...
`SCD41` 和 `SPS30` 都是 I2C，从接线角度看就是：

- 两颗传感器的 `SDA` 都连到 `ESP32 GPIO8`
- 两颗传感器的 `SCL` 都连到 `ESP32 GPIO9`

如果你没法把两根杜邦线同时插进一个孔，需要用下面任意一种分线方式：
...
| `SPS30` | `SDA` | 左侧标 `8` 的孔 | `GPIO8` | 与 SCD41 共用 SDA |
| `SPS30` | `SCL` | 左侧标 `9` 的孔 | `GPIO9` | 与 SCD41 共用 SCL |
...
- `ESP32 5V -> SPS30 VDD`
- `ESP32 GPIO8 -> SPS30 SDA`
- `ESP32 GPIO9 -> SPS30 SCL`
- `ESP32 GND -> SPS30 SEL`
- `ESP32 GND -> SPS30 GND`
