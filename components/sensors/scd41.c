...
static esp_err_t scd41_write_command_word(scd41_t *sensor, uint16_t command, uint16_t value)
{
    uint8_t bytes[5] = {
        (uint8_t)(command >> 8),
        (uint8_t)command,
        (uint8_t)(value >> 8),
        (uint8_t)value,
        scd41_crc_word(value),
    };
    return i2c_master_transmit(sensor->dev_handle, bytes, sizeof(bytes), pdMS_TO_TICKS(I2C_TIMEOUT_MS));
}

static esp_err_t scd41_read_words(scd41_t *sensor, uint16_t command, uint16_t *words, size_t word_count)
{
    uint8_t command_buf[2] = {(uint8_t)(command >> 8), (uint8_t)command};
    uint8_t response[18] = {0};
    size_t response_len = word_count * 3;
    ESP_RETURN_ON_FALSE(response_len <= sizeof(response), ESP_ERR_INVALID_SIZE, "scd41", "response too long");

    ESP_RETURN_ON_ERROR(i2c_master_transmit_receive(sensor->dev_handle,
...
esp_err_t scd41_init(scd41_t *sensor, int i2c_port, int sda_gpio, int scl_gpio)
{
    ...
    ESP_RETURN_ON_ERROR(i2c_new_master_bus(&bus_config, &sensor->bus_handle), "scd41", "new bus failed");
...
{
    uint16_t status = 0;
    ESP_RETURN_ON_ERROR(scd41_read_words(sensor, CMD_GET_DATA_READY_STATUS, &status, 1), "scd41", "data_ready failed");
...
{
    uint16_t words[3] = {0};
    ESP_RETURN_ON_ERROR(scd41_read_words(sensor, CMD_READ_MEASUREMENT, words, 3), "scd41", "read_measurement failed");
    *co2_ppm = words[0];
...
{
    uint16_t response = 0;
    ESP_RETURN_ON_ERROR(scd41_write_command_word(sensor, CMD_PERFORM_FRC, target_ppm), "scd41", "frc write failed");
    vTaskDelay(pdMS_TO_TICKS(400));
    ESP_RETURN_ON_ERROR(scd41_read_words(sensor, CMD_PERFORM_FRC, &response, 1), "scd41", "frc read failed");
