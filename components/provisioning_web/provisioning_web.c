...
static esp_err_t send_error_json(httpd_req_t *req, const char *status, const char *message)
{
    ...
    cJSON_AddStringToObject(root, "message", message != NULL ? message : "请求失败");
...
static esp_err_t send_ota_status_json(httpd_req_t *req)
{
    cJSON *root = cJSON_CreateObject();
    ESP_RETURN_ON_FALSE(root != NULL, ESP_ERR_NO_MEM, TAG, "json alloc failed");
    cJSON_AddStringToObject(root, "status", "ok");
    esp_err_t err = ota_manager_append_status_json(root);
    if (err == ESP_OK) {
...
static esp_err_t send_ota_manager_error_json(httpd_req_t *req,
                                             esp_err_t err,
                                             const char *message,
                                             const char *default_status)
{
    ...
    } else if (err == ESP_ERR_TIMEOUT || err == ESP_ERR_NO_MEM || err == ESP_ERR_NOT_FOUND) {
...
static esp_err_t status_handler(httpd_req_t *req)
{
    ...
    device_state_json_options_t state_options = {
        .include_sample_age_sec = true,
    };
    esp_err_t err = device_state_json_build_sensor_state(snapshot_json,
                                                         &snapshot,
    ...
    err = ota_manager_append_status_json(root);
    if (err == ESP_OK) {
        err = send_json(req, root);
    }
    cJSON_Delete(root);
...
{
    char error[192] = {0};
    esp_err_t err = ota_manager_check_github_release(error, sizeof(error));
    if (err != ESP_OK) {
        return send_ota_manager_error_json(req,
                                           err,
                                           error[0] ? error : "GitHub Release 查询失败",
                                           "502 Bad Gateway");
    }
    return send_ota_status_json(req);
}

static esp_err_t github_ota_update_handler(httpd_req_t *req)
{
    char error[192] = {0};
    esp_err_t err = ota_manager_start_github_update(error, sizeof(error));
    if (err != ESP_OK) {
        return send_ota_manager_error_json(req,
                                           err,
                                           error[0] ? error : "无法启动 GitHub OTA 后台任务",
...
{
    char error[192] = {0};
    esp_err_t err = ota_manager_upload_firmware(req, error, sizeof(error));
    if (err != ESP_OK) {
        return send_ota_manager_error_json(req,
                                           err,
                                           error[0] ? error : "固件上传失败",
                                           "500 Internal Server Error");
    }
                                 ...
                                 void *user_ctx)
{
    ...
    ota_manager_callbacks_t ota_callbacks = {
        .request_restart = callbacks != NULL ? callbacks->request_restart : NULL,
        .user_ctx = user_ctx,
    };
    ESP_RETURN_ON_ERROR(ota_manager_start(&ota_callbacks), TAG, "ota manager start failed");
    ...
    esp_err_t err = httpd_start(&s_ctx.server, &config);
    if (err != ESP_OK) {
        ota_manager_stop();
        return err;
    }
    ...
    for (size_t i = 0; i < sizeof(uris) / sizeof(uris[0]); ++i) {
        err = httpd_register_uri_handler(s_ctx.server, &uris[i]);
        if (err != ESP_OK) {
